...
const Layer1 = ({ layer1FolderName }) => {
  ...
  useEffect(() => {
    getFolders(`Shared Documents/${layer1FolderName}`, sp).then(res => setFolders(res))
  }, []);

  return (
    <Paper variant="outlined" square={false} className={styles.layer1}>
      <div className={styles.heading}>{layer1FolderName}</div>
      <div className={styles.container}>
        ...
        <FilesComponent folderName={`Shared Documents/${layer1FolderName}`} />
      </div>
    </Paper>
  );
};
