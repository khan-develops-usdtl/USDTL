...
const Layer4 = ({ layer1FolderName, layer2FolderName, layer3FolderName, layer4FolderName }) => {
  ...
  useEffect(() => {
    getFolders(`${layer1FolderName}/${layer2FolderName}/${layer3FolderName}/${layer4FolderName}`, sp).then(res => setFolders(res))
  }, []);
    ...
    <div className={styles.layer4}>
      {folders.length > 0 &&
        folders.map((folder) => (
          ...
          <FilesComponent folderName={`${layer1FolderName}/${layer2FolderName}/${layer3FolderName}/${layer4FolderName}`} />
    </div>
  );
};
