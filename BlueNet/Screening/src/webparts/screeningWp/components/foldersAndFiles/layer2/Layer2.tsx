...
import { IFolder } from "./ILayer2";

const Layer2 = ({ layer1FolderName, layer2FolderName }) => {
  const [folders, setFolders] = useState<IFolder[]>([]);

  useEffect(() => {
    getFolders(`${layer1FolderName}/${layer2FolderName}`, sp).then(res => setFolders(res))
  }, []);
  ...
  return (
    <div className={styles.layer2}>
        ...
        <FilesComponent folderName={`${layer1FolderName}/${layer2FolderName}`} />
    </div>
  );
};
