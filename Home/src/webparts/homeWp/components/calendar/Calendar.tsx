...
const Calendar = ({ context }) => {
  ...
  const _getEvents = async () => {
    const calendarsRes = await sp.web.lists.getByTitle('Calendar').items.get()
    const calendars = calendarsRes.map(calendar => ({
      ...calendar,
      id: calendar.ID,
      title: calendar.Title,
      start: calendar.EventDate,
      end: calendar.EndDate,
      url: `${siteUrl}/Lists/Calendar/DispForm.aspx?ID=${calendar.ID}`,
      allDay: calendar.fAllDayEvent
    }))
